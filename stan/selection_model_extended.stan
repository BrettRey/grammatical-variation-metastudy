...
// Extended version: journal × variation-type interaction + year spline (selection stage)
...
data {
  ...
  int<lower=1> K_s;          // predictors for selection (includes interaction, spline)
  int<lower=1> K_o;          // predictors for outcome (main effects only)
  matrix[N, K_s] X_s;        // design matrix for selection
  matrix[N, K_o] X_o;        // design matrix for outcome
  int<lower=1> J_paper;
