...
# Provisional coding assumption: NA means "null" (analyzed, no effect)
gramm_var <- gramm_var |>
  mutate(
    across(
      c(social.significance, production, perception, metalinguistic.behaviors),
      ~ replace_na(.x, "null")
    )
  )
```
...
gramm_var <- gramm_var |>
  mutate(
    ...
    tested = social.significance %in% c("x", "null"),
    ...
    prod_tested = production %in% c("x", "null"),
    prod_found = production == "x",
    perc_tested = perception %in% c("x", "null"),
    perc_found = perception == "x",
    meta_tested = metalinguistic.behaviors %in% c("x", "null"),
