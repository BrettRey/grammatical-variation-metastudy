...
**Implementation note:** The brms multivariate approach with `foundcond` set to `NA` for untested rows drops complete cases, which collapses the selection model. A proper joint selection–outcome model likely requires custom Stan or an alternative formulation (e.g., ordinal 0/1/2) for the primary analysis.
...
**Implementation note:** brms multivariate models drop complete cases when a response is missing. For sparse domains (especially perception), this collapses predictors to single levels. Until a custom Stan selection model is implemented, domain-specific models will be fit as two-stage (tested on all rows; found among tested only).
...
- **Intercept centering:** regularizing intercepts vs data-centered intercepts (sensitivity only).
