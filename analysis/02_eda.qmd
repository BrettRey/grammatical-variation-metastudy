...
## Sensitivity: Counting “Both” in Both Categories

```{r}
#| label: both-sensitivity
both_sensitivity <- gramm_var |>
  mutate(
    is_realization = variation_type_recode %in% c("realization", "both"),
    is_order = variation_type_recode %in% c("order", "both")
  )

summarize_type <- function(flag) {
  type_df <- both_sensitivity |>
    filter(.data[[flag]])

  tibble(
    n = nrow(type_df),
    tested_n = sum(type_df$tested),
    tested_rate = mean(type_df$tested),
    found_n = sum(type_df$found & type_df$tested),
    found_rate = mean(type_df$found[type_df$tested == 1L])
  )
}

tibble(
  type = c("realization+both", "order+both"),
  summarize_type("is_realization"),
  summarize_type("is_order")
)
```

This mirrors the paper’s reporting convention by letting “both” count in each category; the overall pattern (lower testing for order) remains.

## Prior Sensitivity: Regularizing vs Data-Centered
...
priors_regularizing <- c(
  set_prior("normal(0, 1.0)", class = "Intercept", resp = "tested"),
  set_prior("normal(0, 1.0)", class = "Intercept", resp = "foundcond"),
  set_prior("normal(0, 0.5)", class = "b", resp = "tested"),
  set_prior("normal(0, 0.5)", class = "b", resp = "foundcond"),
  set_prior("exponential(3)", class = "sd", resp = "tested"),
  set_prior("exponential(3)", class = "sd", resp = "foundcond")
)
...
fit_prior_regularizing <- brm(
  bf_tested + bf_found + set_rescor(FALSE),
  data = gramm_var,
  family = bernoulli(),
  prior = priors_regularizing,
  sample_prior = "only",
...
pp_check(fit_prior_regularizing, resp = "tested") +
  ggtitle("Prior predictive: tested (regularizing priors)") +
  labs(subtitle = "y = observed, y_rep = simulated")

pp_check(fit_prior_regularizing, resp = "foundcond") +
  ggtitle("Prior predictive: found | tested (regularizing priors)") +
  labs(subtitle = "y = observed, y_rep = simulated")

pp_check(fit_prior_centered, resp = "tested") +
  ggtitle("Prior predictive: tested (centered priors)") +
  labs(subtitle = "y = observed, y_rep = simulated")

pp_check(fit_prior_centered, resp = "foundcond") +
  ggtitle("Prior predictive: found | tested (centered priors)") +
  labs(subtitle = "y = observed, y_rep = simulated")
...
tibble(
  prior = c("regularizing", "regularizing", "centered", "centered"),
  response = c("tested", "foundcond", "tested", "foundcond"),
  q05 = c(
    summarize_prior_rates(fit_prior_regularizing, "tested")[1],
    summarize_prior_rates(fit_prior_regularizing, "foundcond")[1],
    summarize_prior_rates(fit_prior_centered, "tested")[1],
    summarize_prior_rates(fit_prior_centered, "foundcond")[1]
  ),
  q50 = c(
    summarize_prior_rates(fit_prior_regularizing, "tested")[2],
    summarize_prior_rates(fit_prior_regularizing, "foundcond")[2],
    summarize_prior_rates(fit_prior_centered, "tested")[2],
    summarize_prior_rates(fit_prior_centered, "foundcond")[2]
  ),
  q95 = c(
    summarize_prior_rates(fit_prior_regularizing, "tested")[3],
    summarize_prior_rates(fit_prior_regularizing, "foundcond")[3],
...
fit_domain <- function(data, tested_var, found_var) {
  ...
  priors_d <- c(
    set_prior("normal(0, 1.0)", class = "Intercept", resp = "tested_d"),
    set_prior("normal(0, 1.0)", class = "Intercept", resp = "foundcond_d"),
    set_prior("normal(0, 0.5)", class = "b", resp = "tested_d"),
    set_prior("normal(0, 0.5)", class = "b", resp = "foundcond_d"),
    set_prior("exponential(3)", class = "sd", resp = "tested_d"),
    set_prior("exponential(3)", class = "sd", resp = "foundcond_d")
  )
