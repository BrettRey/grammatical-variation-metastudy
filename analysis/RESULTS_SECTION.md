...
Conditional on being tested, evidence for social significance is high across all groups and shows no strong systematic differences by journal or variation type. All covariate effects in the outcome stage have wide intervals spanning 1 (Table 1), and predicted probabilities of finding significance overlap across groups (Figure 3). Wide credible intervals in sparse categories—order variables, perception and metalinguistic domains—reflect data limitations, not absence of effect. The model's partial pooling shrinks extreme estimates toward the population mean, so intervals spanning no effect should be read as "data cannot resolve this", not "effect is absent." This suggests that the primary asymmetry in the literature lies in **which variables get tested**, not in **which tested variables are found to be socially significant**.
