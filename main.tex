...
Results are presented as odds ratios (ORs) with 95\% credible intervals (CrIs). An OR of 2 means twice the odds; an OR of 0.5 means half the odds. A 95\% CrI is an interval containing 95\% of the posterior probability mass for the parameter, given the data, model, and priors. If the interval excludes 1, the posterior strongly favours a non-zero effect on the odds-ratio scale. All fixed effects are treatment-coded with JSlx and \term{both} as reference levels, and year centered at the corpus mean.
...
Order variables are rare and heavily concentrated in LVC~-- the journal that is \emph{less} likely to test for social significance. This makes sense: order variables may appear in LVC for linguistic rather than sociolinguistic analysis. But it constrains journal comparisons for outcome effects. With only 31 tested order variables, power to detect plausible outcome-stage differences is limited; detecting even a 15-percentage-point difference in success rates would require implausibly large samples given the ~90\% baseline. Because the database indexes published studies, the model can't address file-drawer effects where testing occurs but null results aren't published.
